...
exports.getMonthlyFixedCost = getMonthlyFixedCost;
...
function getModelPricing(provider, model) {
    ...
    // Exact match first
    const exact = OPENAI_MODELS.find((p) => p.provider === providerKey && p.model === key);
    if (exact)
        return exact;
    // Prefix match (handles versioned model names like gpt-4o-mini-2024-07-18)
    return OPENAI_MODELS.find((p) => p.provider === providerKey && key.startsWith(p.model));
}
