...
import type {
  CorrelatedSnapshot,
  ParsedTradingviewAlert,
  RawTradingviewPayload,
  TradePlan,
  TradingContext,
  TradingviewAlertRecord,
  TradingviewSymbol,
  UpcomingUsdEvent,
} from '../types/tradingview.js';

const YAHOO_BASE = 'https://query1.finance.yahoo.com/v8/finance/chart';
const SIGNAL_TYPES = new Set(['setup_detected', 'important_zone', 'price_zone_reached']);
const SYMBOL_ALIASES: Record<string, TradingviewSymbol> = {
  XAUUSD: 'XAUUSD',
  EURUSD: 'EURUSD',
  GBPUSD: 'GBPUSD',
  USDJPY: 'USDJPY',
...
function getConfig(): TradingConfig {
...
function getStorageFile() {
  return process.env.TRADING_ALERTS_FILE ?? path.join(process.cwd(), 'backend', 'server', 'data', 'tradingview-alerts.json');
}
...
async function readRecords(): Promise<TradingviewAlertRecord[]> {
  const filePath = await ensureStorageFile();
  const raw = await readFile(filePath, 'utf8');
  return JSON.parse(raw) as TradingviewAlertRecord[];
}

async function writeRecords(records: TradingviewAlertRecord[]) {
  const filePath = await ensureStorageFile();
...
function normalizeSymbol(raw: string): TradingviewSymbol | null {
...
function normalizeSignalType(value: string | null, signal: string | null): ParsedTradingviewAlert['signalType'] {
  if (value && SIGNAL_TYPES.has(value)) return value as ParsedTradingviewAlert['signalType'];
...
function normalizeDirectionHint(value: string | null, signal: string | null): ParsedTradingviewAlert['directionHint'] {
...
export function parseTradingviewPayload(payload: RawTradingviewPayload): ParsedTradingviewAlert {
  ...
  const trend = (toStringValue(payload.trend) ?? 'ranging') as ParsedTradingviewAlert['trend'];
  const structure = (toStringValue(payload.structure) ?? 'none') as ParsedTradingviewAlert['structure'];
  const candlePattern = (toStringValue(payload.candle_pattern) ?? 'none') as ParsedTradingviewAlert['candlePattern'];
  const liquidityEvent = (toStringValue(payload.liquidity_event) ?? 'none') as ParsedTradingviewAlert['liquidityEvent'];
  ...
  return {
    ...
    price,
    eventTimeIso,
    signalType: signalType as ParsedTradingviewAlert['signalType'],
    directionHint,
    trend,
...
async function fetchCorrelatedSnapshot(spec: PriceSpec): Promise<CorrelatedSnapshot> {
  type YahooResponse = {
    chart?: {
      result?: Array<{
  ...
  try {
    const data = await fetchJson<YahooResponse>(`${YAHOO_BASE}/${encodeURIComponent(spec.symbol)}?range=5d&interval=60m`);
    ...
    let trend: CorrelatedSnapshot['trend'] = 'unknown';
    if (closes.length >= 4) {
...
function detectSession(alert: ParsedTradingviewAlert): string {
...
async function fetchUpcomingUsdEvents(): Promise<UpcomingUsdEvent[]> {
  const now = new Date();
  ...
  return events
    ...
    .map((event) => {
      ...
      } satisfies UpcomingUsdEvent;
    })
    .filter((event) => event.minutesUntil >= -15 && event.minutesUntil <= 8 * 60)
    .sort((a, b) => a.startsAt.localeCompare(b.startsAt));
}

async function buildTradingContext(alert: ParsedTradingviewAlert): Promise<TradingContext> {
  ...
  let upcomingUsdNews: UpcomingUsdEvent[] = [];
  try {
...
function buildPrompt(alert: ParsedTradingviewAlert, context: TradingContext, config: TradingConfig) {
...
function defaultAtrForSymbol(symbol: TradingviewSymbol, price: number) {
  if (symbol === 'XAUUSD') return Math.max(6, price * 0.0025);
  if (symbol === 'USDJPY') return Math.max(0.18, price * 0.0012);
  return Math.max(0.0012, price * 0.0009);
}

function roundPrice(symbol: TradingviewSymbol, value: number) {
  const decimals = symbol === 'XAUUSD' ? 2 : symbol === 'USDJPY' ? 3 : 5;
  return Number(value.toFixed(decimals));
}

function buildHeuristicTradePlan(
  ...
  let decision: TradePlan['decision'] = 'NO_TRADE';
...
async function completeStructuredTradePlan(alert: ParsedTradingviewAlert, context: TradingContext, config: TradingConfig): Promise<{ plan: TradePlan; raw: string }> {
  ...
  const plan = await chatCompleteJSON<TradePlan>(
    [
...
async function generateTradePlan(alert: ParsedTradingviewAlert, context: TradingContext, config: TradingConfig) {
  try {
...
function computeRiskReward(plan: TradePlan): number | null {
...
function alignsWithDirection(plan: TradePlan) {
  const entryLow = toNumeric(plan.entry_zone?.low);
  const entryHigh = toNumeric(plan.entry_zone?.high);
  const stop = toNumeric(plan.stop_loss);
  const tp1 = toNumeric(plan.take_profit_1);
...
function enforceRiskGuards(planRaw: TradePlan, alert: ParsedTradingviewAlert, context: TradingContext, config: TradingConfig): TradePlan {
  const fallbackBias = normalizedDecisionToBias(planRaw.decision);
  const normalized: TradePlan = {
...
function normalizedDecisionToBias(decision: string | undefined): 'bullish' | 'bearish' | 'neutral' {
  if (decision === 'BUY') return 'bullish';
  if (decision === 'SELL') return 'bearish';
  return 'neutral';
}

function formatNotification(alert: ParsedTradingviewAlert, plan: TradePlan, context: TradingContext) {
  return [
    `PAIR: ${plan.symbol}`,
...
async function sendNotifications(alert: ParsedTradingviewAlert, plan: TradePlan, context: TradingContext) {
  const message = formatNotification(alert, plan, context);
  const deliveries: TradingviewAlertRecord['notifications'] = [];
...
async function saveRecord(record: TradingviewAlertRecord) {
...
export async function listRecentTradingviewAlerts(limit = 30) {
  const records = await readRecords();
  return records.sort((a, b) => b.receivedAt.localeCompare(a.receivedAt)).slice(0, limit);
}

export async function processTradingviewWebhook(payload: RawTradingviewPayload) {
  ...
  const baseRecord: TradingviewAlertRecord = {
    id: randomUUID(),
  ...
  try {
    ...
    const processedRecord: TradingviewAlertRecord = {
      ...baseRecord,
      status: 'processed',
      context,
      analysis: finalPlan,
    ...
    const failedRecord: TradingviewAlertRecord = {
      ...baseRecord,
      status: 'failed',
...
export async function analyzeTradingSignal(payload: RawTradingviewPayload) {
  const config = getConfig();
  const alert = parseTradingviewPayload(payload);
  const context = await buildTradingContext(alert);
  const { plan: rawPlan, raw } = await generateTradePlan(alert, context, config);
