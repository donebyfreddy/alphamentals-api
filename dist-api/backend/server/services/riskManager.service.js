...
function calculateRisk(input) {
    ...
    let suggestion = '';
    if (rrRatio < 1)
        suggestion = 'WARNING: RR below 1:1 — this trade does not meet minimum standards.';
    else if (rrRatio < 2)
        suggestion = 'Acceptable RR. Consider if the setup justifies a sub-2R trade.';
    else if (rrRatio >= 3)
        suggestion = 'Excellent RR. Ensure entry is precise — wide stops dilute quality.';
    else
        suggestion = 'Good RR. Risk is well-defined.';
    if (riskPercent > 2)
        suggestion += ' CAUTION: Risk exceeds 2% per trade recommendation.';
    return {
