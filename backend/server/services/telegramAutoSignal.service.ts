import { getPreferences, createNotification } from './notification.service.js';
import { sendMail } from '../lib/mailer.js';
import type { TelegramMessageRecord } from '../types/telegram.js';
import { getTelegramMessageById, getTelegramMessageBySignalHash, updateTelegramMessageAutomation } from './telegramMessageStore.service.js';
import { validateTelegramTradeSignal, type TelegramTradeValidationResult } from './telegramSignalAnalyze.service.js';

const TELEGRAM_ALERT_RECIPIENT = 'fo.mencuccini@gmail.com';
const AUTO_SIGNAL_TIMEOUT_MS = 120_000;

function supportedAutoSignal(message: TelegramMessageRecord) {
  return (
...
function computeEmailSubject(result: TelegramTradeValidationResult) {
...
function formatDateGmt(value: string) {
...
function decisionLabel(result: TelegramTradeValidationResult) {
  if (result.verdict === 'GOOD') return { text: 'APPROVED', emoji: '🟢', color: '#34d399', bg: '#052e24', border: '#10b981' };
  if (result.verdict === 'BAD') return { text: 'REJECTED', emoji: '🔴', color: '#f87171', bg: '#3f1218', border: '#ef4444' };
  if (result.verdict === 'RISKY') return { text: 'RISKY', emoji: '🟠', color: '#fb923c', bg: '#431407', border: '#f97316' };
  return { text: 'MONITOR', emoji: '🟡', color: '#fde047', bg: '#3a2f08', border: '#eab308' };
}

function actionLabel(result: TelegramTradeValidationResult) {
  if (result.finalAction === 'take') return 'APPROVED';
  if (result.finalAction === 'avoid') return 'REJECTED';
  return 'MONITOR';
}

function statusChip(label: string | null | undefined, tone: 'green' | 'yellow' | 'red' | 'blue' = 'blue') {
  if (!label) return '';
  const colors = {
...
function statusTone(value: string | null | undefined): 'green' | 'yellow' | 'red' {
  const normalized = String(value ?? '').toLowerCase();
  if (['good', 'approved', 'aligned', 'normal', 'low', 'safe', 'healthy'].includes(normalized)) return 'green';
  if (['high', 'extreme', 'against', 'avoid', 'bad', 'rejected'].includes(normalized)) return 'red';
  return 'yellow';
}

function metric(label: string, value: string | number | null | undefined, icon = '') {
...
function metricGrid(items: Array<[string, string | number | null | undefined, string?]>) {
...
function bulletList(items: string[], icon = '•') {
...
function buildReasoningGroups(result: TelegramTradeValidationResult) {
  const positives = [
    ...result.keyReasons,
...
function scoreBar(value: number | null | undefined) {
...
function confluenceRows(result: TelegramTradeValidationResult) {
  ...
  return [
    ...
    <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin:9px 0;font-size:13px;">
      ...
      <span>${scoreBar(value as number)}</span>
    </div>
  `).join('');
}

function rrChips(result: TelegramTradeValidationResult) {
  if (!result.rr?.targets.length) return '';
...
function section(title: string, body: string, accent = '#1f2937') {
  if (!body.trim()) return '';
  return `
...
function buildTelegramSignalEmail(message: TelegramMessageRecord, result: TelegramTradeValidationResult) {
 ...
 */
export async function sendEmailForMessage(messageId: string): Promise<
  | { sent: true; verdict: string; confidence: number }
...
> {
  ...
  let validation: TelegramTradeValidationResult | null = null;
  ...
  if (message.autoAnalysisStatus === 'completed' || message.autoAnalysisStatus === 'fallback') {
    ...
    if (saved && typeof saved === 'object' && 'ok' in saved && saved.ok === true) {
      ...
      validation = saved as any as TelegramTradeValidationResult;
    }
  }
  ...
  if (!validation) {
    ...
    await updateTelegramMessageAutomation(message.id, {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      autoAnalysisResult: validation as any as Record<string, unknown>,
...
export async function handleNewTelegramSignal(message: TelegramMessageRecord) {
  if (!supportedAutoSignal(message)) return { skipped: true, reason: 'Message is not a supported trading signal.' } as const;
  if (message.emailSentAt) return { skipped: true, reason: 'Signal email already sent.' } as const;
  if (message.autoAnalysisStatus === 'running') return { skipped: true, reason: 'Signal analysis already running.' } as const;
  if (message.autoAnalysisStatus === 'failed') return { skipped: true, reason: 'Automatic analysis previously failed; manual retry only.' } as const;
  ...
  if (message.signalHash) {
    ...
    if (duplicate && duplicate.id !== message.id && duplicate.emailSentAt) {
      ...
      return { skipped: true, reason: 'Duplicate signal hash already processed.' } as const;
    }
  }
  ...
  try {
    return await withTimeout((async () => {
      ...
      if (validation.ok === false) {
        ...
        return { skipped: true, reason: validation.error } as const;
      }
      ...
      await updateTelegramMessageAutomation(message.id, {
        autoAnalysisStatus: validation.aiValidationUnavailable ? 'fallback' : 'completed',
        autoAnalysisResult: validation as unknown as Record<string, unknown>,
        autoAnalysisError: validation.aiValidationError ?? null,
        autoAnalysisAt: now,
        emailSentAt: emailResult.ok ? now : null,
        emailStatus: emailResult.ok ? 'sent' : 'failed',
      ...
      return { skipped: false, sent: emailResult.ok, validation } as const;
    ...
    return { skipped: true, reason: messageText } as const;
  }
}
