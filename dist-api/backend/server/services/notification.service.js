...
exports.CATEGORY_LABELS = exports.NOTIFICATION_CATEGORIES = void 0;
...
function defaultPreferences(userId) {
    return {
        ...
        enabledEmailCategories: [...exports.NOTIFICATION_CATEGORIES],
        webhookEnabled: false,
        webhookUrl: null,
        webhookSecret: null,
        enabledWebhookCategories: [...exports.NOTIFICATION_CATEGORIES],
    };
}
async function getPreferences(userId) {
    try {
        ...
        return { ...defaultPreferences(userId), ...data, userId };
    }
    catch {
        return defaultPreferences(userId);
    }
...
function buildEmailHtml(ctx) {
    ...
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:28px;background:#0f1117;color:#e5e7eb;border-radius:10px;">
      ...
      <p style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:#9ca3af;margin:0 0 4px;">${exports.CATEGORY_LABELS[ctx.category]}${symbolRow}</p>
...
async function createNotification(input) {
    ...
    try {
        ...
        if (input.dedupeKey) {
            ...
            const { data: dupes } = await supabase_js_1.supabase
        ...
        const emailEligible = prefs.emailEnabled && !!prefs.emailRecipient && meetsSeverity &&
            prefs.emailFrequency === 'instant' && prefs.enabledEmailCategories.includes(input.category);
        const webhookEligible = prefs.webhookEnabled && (0, webhook_js_1.isValidWebhookUrl)(prefs.webhookUrl) &&
        ...
        const { data: row, error } = await supabase_js_1.supabase.from('notifications').insert({
        ...
        // Webhook delivery
        if (webhookEligible) {
            const result = await (0, webhook_js_1.sendWebhook)(prefs.webhookUrl, prefs.webhookSecret ?? undefined, {
                app: 'AlphaMentals',
        ...
        if (emailEligible || webhookEligible) {
            await supabase_js_1.supabase.from('notifications').update({
...
async function markRead(userId, id) {
    const { error } = await supabase_js_1.supabase.from('notifications').update({ read: true }).eq('id', id).eq('userId', userId);
    if (error)
        throw new Error(error.message);
}
async function markAllRead(userId) {
    const { error } = await supabase_js_1.supabase.from('notifications').update({ read: true }).eq('userId', userId).eq('read', false);
    if (error)
        throw new Error(error.message);
}
async function clearHistory(userId) {
    const { error } = await supabase_js_1.supabase.from('notifications').delete().eq('userId', userId);
    if (error)
        throw new Error(error.message);
}
// ── Test actions ─────────────────────────────────────────────────────────────
const DEFAULT_ALERT_RECIPIENT = 'fo.mencuccini@gmail.com';
async function sendTestEmail(userId, recipientOverride) {
    const prefs = await getPreferences(userId);
    ...
    const result = await (0, mailer_js_1.sendMail)({
        ...
        html: buildEmailHtml({
            ...
            metadata: { mode: (0, mailer_js_1.getMailMode)(), provider: 'resend' },
        }),
...
async function sendTestWebhook(userId, urlOverride, secretOverride) {
    const prefs = await getPreferences(userId);
    const url = urlOverride || prefs.webhookUrl;
    const secret = secretOverride ?? prefs.webhookSecret ?? undefined;
    if (!(0, webhook_js_1.isValidWebhookUrl)(url))
        return { success: false, message: 'No valid webhook URL set.' };
    const result = await (0, webhook_js_1.sendWebhook)(url, secret, {
        app: 'AlphaMentals',
        title: 'Test Webhook',
