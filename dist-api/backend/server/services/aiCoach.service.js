...
exports.reviewTrade = reviewTrade;
exports.generateWeeklyCoaching = generateWeeklyCoaching;
exports.generateDailyDebrief = generateDailyDebrief;
...
async function reviewTrade(userId, tradeId) {
    const { data: trade, error } = await supabase_js_1.supabase
...
NEXT IMPROVEMENT: [one concrete, actionable thing to do on the next trade]`;
    ...
    const { error: updErr } = await supabase_js_1.supabase.from('trades').update({
...
async function generateWeeklyCoaching(userId, weekStr) {
    const now = new Date();
    ...
    const { trades: tradesResult } = await (0, tradeJournal_service_js_1.getTrades)(userId, { from: weekStart.toISOString(), to: weekEnd.toISOString(), limit: 50 });
    const stats = await (0, tradeJournal_service_js_1.getPerformanceStats)(userId, weekStart.toISOString(), weekEnd.toISOString());
    const patterns = await (0, analytics_service_js_1.detectMistakePatterns)(userId);
    const { data: profile } = await supabase_js_1.supabase.from('user_profiles').select('*').eq('userId', userId).single();
...
Cover: performance reality, behavioral patterns, top 3 strengths, top 3 fixes, one drill for next week, mindset note.`;
    ...
    await supabase_js_1.supabase.from('coaching_sessions').insert({
...
// ─── Daily debrief ────────────────────────────────────────────────────────
async function generateDailyDebrief(userId, date) {
    const day = date ?? new Date().toISOString().slice(0, 10);
    const from = `${day}T00:00:00.000Z`;
    const to = `${day}T23:59:59.999Z`;
    const { trades: tradesResult } = await (0, tradeJournal_service_js_1.getTrades)(userId, { from, to, limit: 20 });
    const stats = await (0, tradeJournal_service_js_1.getPerformanceStats)(userId, from, to);
    const patterns = await (0, analytics_service_js_1.detectMistakePatterns)(userId);
    if (stats.totalTrades === 0)
        return 'No trades today. Rest days are part of the process.';
...
// ─── Ask the coach ─────────────────────────────────────────────────────────
async function askCoach(userId, question) {
    const stats = await (0, tradeJournal_service_js_1.getPerformanceStats)(userId);
    const patterns = await (0, analytics_service_js_1.detectMistakePatterns)(userId);
    const { data: profile } = await supabase_js_1.supabase.from('user_profiles').select('*').eq('userId', userId).single();
