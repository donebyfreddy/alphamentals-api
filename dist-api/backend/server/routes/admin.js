...
exports.adminRouter.post('/fundamentals-ai/run-now', async (req, res) => {
    if (!isAdminAuthorized(req)) {
        return res.status(401).json({ error: 'Unauthorized admin request' });
    }
    try {
        const result = await (0, aiAnalysisRuns_service_js_1.runAiAnalysis)({ trigger: 'manual', bypassCooldown: true });
        return res.json({
            success: result.ok,
            runType: 'manual',
