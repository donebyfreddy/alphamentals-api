...
import type { EquityCurvePoint, HeatmapCell, MistakeBreakdown, SetupPerformance, PsychologyCorrelation, SetupQualityPerformance, MistakeCost, DisciplineStats, RiskFlagStats, TimeOfDayCell } from '../types/index.js';
...
export async function getEquityCurve(userId: string, from?: string, to?: string): Promise<EquityCurvePoint[]> {
  let query = supabase.from('trades').select('entryTime, pnl').eq('userId', userId).eq('status', 'CLOSED').order('entryTime', { ascending: true });
...
export async function getSessionHeatmap(userId: string): Promise<HeatmapCell[]> {
...
export async function getDayOfWeekHeatmap(userId: string): Promise<HeatmapCell[]> {
...
export async function getMistakeBreakdown(userId: string): Promise<MistakeBreakdown[]> {
  const { data: trades } = await supabase
    .from('trades').select('mistakeTags, pnl, executionScore')
    .eq('userId', userId).eq('status', 'CLOSED').eq('reviewStatus', 'COMPLETE')
    .not('mistakeTags', 'eq', '{}');

  const map = new Map<string, { count: number; pnl: number; scores: number[] }>();
  for (const t of trades ?? []) {
...
export async function getSetupPerformance(userId: string): Promise<SetupPerformance[]> {
  const { data: trades } = await supabase
...
export async function getPsychologyCorrelations(userId: string): Promise<PsychologyCorrelation[]> {
  const { data: trades } = await supabase
...
export async function detectMistakePatterns(userId: string): Promise<string[]> {
  ...
  let streak = 0;
  for (const x of t) {
    if ((x.pnl ?? 0) < 0) { streak++; } else { streak = 0; }
  }
...
export async function getSetupQualityPerformance(userId: string): Promise<SetupQualityPerformance[]> {
  const { data: trades } = await supabase
    .from('trades').select('setupQualityGrade, pnl, rrActual, disciplineScore, psychologyScore')
    .eq('userId', userId).eq('status', 'CLOSED').eq('reviewStatus', 'COMPLETE')
    .not('setupQualityGrade', 'is', null);

  const map = new Map<string, { wins: number; total: number; pnl: number; rr: number[]; disc: number[]; psy: number[] }>();
  for (const t of trades ?? []) {
...
export async function getMistakeCost(userId: string): Promise<MistakeCost[]> {
  const { data: trades } = await supabase
...
export async function getDisciplineStats(userId: string): Promise<DisciplineStats> {
  const { data: trades } = await supabase
    .from('trades').select('followedPlan, blueprintMatchScore, blueprintRulesBroken, pnl')
    .eq('userId', userId).eq('status', 'CLOSED').eq('reviewStatus', 'COMPLETE');
  const t = trades ?? [];

  const bucket = (rows: { pnl: number | null }[]) => {
  ...
  let mostBrokenRule: string | null = null;
  let maxBroken = 0;
  for (const [rule, c] of brokenCount) {
    if (c > maxBroken) { maxBroken = c; mostBrokenRule = rule; }
  }
...
export async function getRiskFlagStats(userId: string): Promise<RiskFlagStats> {
  const { data: trades } = await supabase
    .from('trades').select('stopLoss, entryPrice, takeProfit, rrPlanned, movedStopLoss, mistakeTags, riskPercent, pnl')
    .eq('userId', userId).eq('status', 'CLOSED').eq('reviewStatus', 'COMPLETE');
  const t = trades ?? [];

  const tally = (predicate: (x: typeof t[number]) => boolean) => {
    const rows = t.filter(predicate);
    return { count: rows.length, pnl: Math.round(rows.reduce((s, x) => s + (x.pnl ?? 0), 0) * 100) / 100 };
  };

  const slMissing = (x: typeof t[number]) => x.stopLoss == null || x.stopLoss === 0 || x.stopLoss === x.entryPrice;
  const hasTag = (x: typeof t[number], ...names: string[]) => (x.mistakeTags ?? []).some((m: string) => names.includes(m));
...
export async function getTimeOfDayPerformance(userId: string): Promise<TimeOfDayCell[]> {
  const { data: trades } = await supabase
    .from('trades').select('entryTime, pnl')
    .eq('userId', userId).eq('status', 'CLOSED').eq('reviewStatus', 'COMPLETE');
...
export async function getPsychologyByPhase(userId: string, phase: 'pre' | 'during' | 'post'): Promise<PsychologyCorrelation[]> {
  const columnByPhase: Record<string, string> = {
...
export async function getPerformanceBySymbol(userId: string): Promise<SymbolPerformance[]> {
  const { data: trades } = await supabase
  ...
  for (const t of trades ?? []) {
    ...
    e.pnl += t.pnl ?? 0;
    if ((t.pnl ?? 0) > 0) e.wins++;
    if (t.rrActual != null) e.rr.push(t.rrActual as number);
    map.set(sym, e);
  }
...
export interface GoodVsBadLossStats {
  validLosses: LossBucket;
  badLosses: LossBucket;
  undisciplinedWins: number;
}

export async function getGoodVsBadLossStats(userId: string): Promise<GoodVsBadLossStats> {
  const { data: trades } = await supabase
  ...
  const buildBucket = (rows: { pnl: number | null }[]): LossBucket => {
...
export interface PsychologyFlagCost {
  flag: string;
  tradeCount: number;
  totalPnl: number;
  avgPnl: number;
...
export async function getPsychologyFlagCost(userId: string): Promise<PsychologyFlagCost[]> {
  const { data: trades } = await supabase
  ...
  return flags.map(({ key, label }) => {
    const rows = t.filter(x => (x as Record<string, unknown>)[key] === true);
    const count = rows.length;
    const totalPnl = Math.round(rows.reduce((s, x) => s + (x.pnl ?? 0), 0) * 100) / 100;
    const avgPnl = count > 0 ? Math.round((totalPnl / count) * 100) / 100 : 0;
    const wins = rows.filter(x => (x.pnl ?? 0) > 0).length;
...
export async function getReviewCoverage(userId: string): Promise<ReviewCoverage> {
  const { data: trades } = await supabase
