...
exports.setSyncScheduleMetadata = setSyncScheduleMetadata;
exports.ingestTelegramMessage = ingestTelegramMessage;
exports.getTelegramStatus = getTelegramStatus;
exports.getTelegramConnectionTest = getTelegramConnectionTest;
exports.syncTelegramSignals = syncTelegramSignals;
exports.syncTelegramMessages = syncTelegramMessages;
exports.syncTelegramMessagesScheduled = syncTelegramMessagesScheduled;
exports.getRecentTelegramMessages = getRecentTelegramMessages;
exports.analyzeTelegramMessage = analyzeTelegramMessage;
exports.normalizeTelegramRouteError = normalizeTelegramRouteError;
const telegramBridge_service_js_1 = require("./telegramBridge.service.js");
...
const tradingviewBridge_service_js_1 = require("./tradingviewBridge.service.js");
const telegramAutoSignal_service_js_1 = require("./telegramAutoSignal.service.js");
...
function logTelegramSyncStep(message, context = {}) {
    const payload = Object.fromEntries(Object.entries(context).filter(([, value]) => value !== undefined && value !== null));
...
async function ingestTelegramMessage(payload, strategy) {
    const parsed = (0, parseTelegramSignal_js_1.parseTelegramSignal)(payload.rawText);
    const stored = await (0, telegramMessageStore_service_js_1.storeTelegramMessage)({
...
async function getTelegramStatus() {
    const [counts, connection] = await Promise.all([
        (0, telegramMessageStore_service_js_1.getTelegramMessageCounts)().catch(() => ({ total: 0, signals: 0, latestSync: null })),
        (0, telegramBridge_service_js_1.testTelegramConnection)(),
    ]);
    const runtime = (0, telegramBridge_service_js_1.getTelegramRuntimeState)();
...
async function getTelegramConnectionTest() {
    return await (0, telegramBridge_service_js_1.testTelegramConnection)();
}
async function runTelegramSync(limit, source) {
    ...
    const latestMessageId = targetChat ? await (0, telegramMessageStore_service_js_1.getLatestTelegramMessageIdForChat)(targetChat).catch(() => null) : null;
    if (latestMessageId) {
    ...
    const result = await (0, telegramBridge_service_js_1.fetchTelegramHistory)(clampedLimit, latestMessageId);
    ...
    const storageStrategy = await (0, telegramMessageStore_service_js_1.resolveStorageStrategy)(true);
    if (storageStrategy === 'unavailable') {
        throw new telegramBridge_service_js_1.TelegramBridgeError('TELEGRAM_UNAVAILABLE', 'Telegram messages were fetched but could not be saved to the database.', 'Database unreachable: configure DATABASE_URL or SUPABASE_SERVICE_ROLE_KEY.', {
            phase: 'database_save',
    ...
    for (const [index, payload] of messages.entries()) {
        ...
        try {
            ...
            const isValidSignal = stored.parsed.messageType === 'SIGNAL' && (0, parseTelegramSignal_js_1.isTelegramLimitOrderSignal)(payload.rawText, stored.parsed);
            if (stored.imported) {
                imported += 1;
                if (isValidSignal) {
                    newSignals += 1;
                    if (stored.record) {
                        ...
                        void (0, telegramAutoSignal_service_js_1.handleNewTelegramSignal)(signalRecord).then((automation) => {
...
async function syncTelegramMessagesScheduled(limit = 10) {
    try {
...
async function getRecentTelegramMessages(filter) {
    ...
    const messages = await (0, telegramMessageStore_service_js_1.listRecentTelegramMessages)(filter);
    const limitMessages = messages.filter((message) => (0, parseTelegramSignal_js_1.isTelegramLimitOrderSignal)(message.rawText, {
...
async function analyzeTelegramMessage(messageId) {
    ...
    const parsedOverride = {
        ...
        orderType: message.parsedSignal?.orderType ?? undefined,
    ...
    const validated = await (0, telegramSignalAnalyze_service_js_1.validateTelegramTradeSignal)(message.rawText, parsedOverride, {
    ...
    try {
        const response = await (0, tradingviewBridge_service_js_1.analyzeTradingSignal)({
...
function derivePhaseMessage(args) {
    ...
    if (p.includes('target_chat') || p.includes('resolve'))
        return 'Target chat could not be resolved';
    if (p.includes('save') || p.includes('database') || p.includes('store'))
        return 'Messages fetched but failed to save to database';
...
function normalizeTelegramRouteError(error) {
    if (error instanceof telegramBridge_service_js_1.TelegramBridgeError) {
        return {
            status: error.status,
    ...
    if ((0, telegramMessageStore_service_js_1.isTelegramStoreUnavailable)(error)) {
        return {
            status: 503,
    ...
    if (error instanceof Error)
        return { status: 500, message: error.message, rawMessage: error.message, code: 'INTERNAL_ERROR', stack: error.stack, hints: [] };
