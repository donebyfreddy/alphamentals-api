...
exports.listSetups = listSetups;
exports.getSetup = getSetup;
exports.createSetup = createSetup;
exports.updateSetup = updateSetup;
exports.deleteSetup = deleteSetup;
exports.seedDefaultSetups = seedDefaultSetups;
exports.recomputeSetupStats = recomputeSetupStats;
...
async function listSetups(userId, includeInactive = false) {
    let query = supabase_js_1.supabase.from('playbook_setups').select('*').eq('userId', userId);
    if (!includeInactive)
...
async function getSetup(userId, id) {
...
async function createSetup(userId, input) {
    const { data, error } = await supabase_js_1.supabase
...
async function updateSetup(userId, id, input) {
...
async function deleteSetup(userId, id) {
    await getSetup(userId, id);
    const { error } = await supabase_js_1.supabase.from('playbook_setups').delete().eq('id', id).eq('userId', userId);
    if (error)
        throw new Error(error.message);
}
async function archiveSetupIds(userId, ids) {
    if (ids.length === 0)
        return;
...
async function upsertCanonicalSetup(userId, existing, setup) {
 ...
 */
async function seedDefaultSetups(userId) {
    ...
    const { data: refreshed, error: refreshErr } = await supabase_js_1.supabase
        .from('playbook_setups')
        .select('*')
        .eq('userId', userId)
        .eq('isActive', true)
 ...
 */
async function recomputeSetupStats(userId, setupId) {
    ...
    if (total === 0) {
        await supabase_js_1.supabase.from('playbook_setups')
    ...
    await supabase_js_1.supabase.from('playbook_setups').update({
        totalTrades: total,
