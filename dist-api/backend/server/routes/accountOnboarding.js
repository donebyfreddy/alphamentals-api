...
exports.accountOnboardingRouter = void 0;
...
exports.accountOnboardingRouter = (0, express_1.Router)();
const serviceStateSchema = zod_1.z.enum(['connected', 'connecting', 'error', 'disconnected', 'unavailable']);
const connectExistingSchema = zod_1.z.object({
    broker: zod_1.z.string().trim().min(1),
    platform: zod_1.z.literal('MT5'),
    login: zod_1.z.string().trim().min(1),
    password: zod_1.z.string().min(1),
    server: zod_1.z.string().trim().min(1),
});
const createDemoSchema = zod_1.z.object({
    broker: zod_1.z.enum(['Admirals', 'XM', 'ActivTrades', 'Tickmill', 'Pepperstone', 'IC Markets', 'FP Markets', 'Eightcap']),
    firstName: zod_1.z.string().trim().min(1),
    lastName: zod_1.z.string().trim().min(1),
    email: zod_1.z.string().email(),
    country: zod_1.z.string().trim().min(1),
    leverage: zod_1.z.string().trim().min(1),
    startingBalance: zod_1.z.number().positive().optional(),
});
...
async function getQuoteFeedStatus() {
    if (!mt5BridgeClient_js_1.mt5BridgeClient.isConfigured()) {
    ...
    try {
        await mt5BridgeClient_js_1.mt5BridgeClient.get('/quotes?symbols=XAUUSD');
        return serviceStatus({
            state: 'connected',
...
function bridgeServiceStatus(now) {
...
function metaApiServiceStatus(result, now) {
    if (result.success) {
        return serviceStatus({
            state: 'connected',
...
function heartbeatServiceStatus(success, now, message) {
    return serviceStatus({
...
exports.accountOnboardingRouter.post('/mt5/connect-existing', async (req, res) => {
    ...
    const connectResult = await (0, metaTrader_service_js_1.connectMetaTrader)({
        version: 'mt5',
...
exports.accountOnboardingRouter.post('/mt5/create-demo', async (req, res) => {
