...
exports.buildObjectiveValidationFacts = buildObjectiveValidationFacts;
exports.validateTelegramTradeSignal = validateTelegramTradeSignal;
exports.analyzeSignalWithAI = analyzeSignalWithAI;
const gemini_js_1 = require("../lib/gemini.js");
const parseTelegramSignal_js_1 = require("../../../src/utils/telegram/parseTelegramSignal.js");
const pairDecisionContext_service_js_1 = require("./pairDecisionContext.service.js");
...
function classifySpread(symbol, spread) {
    ...
    if (spread >= baseline * 2.5)
        return 'high';
    if (spread >= baseline * 1.5)
        return 'elevated';
    return 'normal';
}
function classifyVolatility(symbol, currentPrice, high, low) {
    if (!isFiniteNumber(currentPrice) || !isFiniteNumber(high) || !isFiniteNumber(low) || currentPrice <= 0)
        return 'unavailable';
    const rangePct = ((high - low) / currentPrice) * 100;
    const isGold = compactSymbol(symbol) === 'XAUUSD';
    if (rangePct >= (isGold ? 1.35 : 0.9))
        return 'extreme';
    if (rangePct >= (isGold ? 0.85 : 0.55))
        return 'high';
    if (rangePct <= (isGold ? 0.2 : 0.12))
        return 'low';
    return 'normal';
}
...
function scoreExecution(args) {
    if (args.hardReject)
        return 0;
    let score = 82;
    if (args.spreadStatus === 'elevated')
        score -= 15;
    if (args.spreadStatus === 'dangerous')
        score -= 35;
    if (args.spreadStatus === 'unavailable')
        score -= 10;
    if (args.volatility === 'high')
        score -= 15;
    if (args.volatility === 'extreme')
        score -= 28;
    if (args.volatility === 'unavailable')
        score -= 8;
    if (args.calendarRisk === 'medium')
        score -= 12;
    if (args.calendarRisk === 'high')
        score -= 24;
    if (args.freshnessStatus === 'Delayed')
        score -= 8;
    if (args.freshnessStatus === 'Stale')
        score -= 28;
    if (args.freshnessStatus === 'Expired')
        score -= 45;
...
function deriveSignalFreshness(symbol, signalTime, currentPrice, parsed, rr, invalidated) {
    ...
    if (signalAgeMs != null) {
        if (signalAgeMs > (isGold ? 6 : 12) * 60 * 60_000)
            freshnessStatus = 'Expired';
        else if (signalAgeMs > (isGold ? 90 : 180) * 60_000)
            freshnessStatus = 'Stale';
        else if (signalAgeMs > (isGold ? 30 : 60) * 60_000)
            freshnessStatus = 'Delayed';
    }
    if (distanceR != null) {
        if (distanceR >= (isGold ? 2 : 2.5))
            freshnessStatus = 'Expired';
        else if (distanceR >= (isGold ? 1.25 : 1.75) && freshnessStatus === 'Fresh')
            freshnessStatus = 'Stale';
        else if (distanceR >= (isGold ? 0.75 : 1.1) && freshnessStatus === 'Fresh')
            freshnessStatus = 'Delayed';
    }
    return {
        signalAgeMs,
...
function deriveSupportResistanceConflict(parsed, pairContext, rr) {
    ...
    if (parsed.direction === 'SELL') {
        ...
        if (distanceToSupport <= 0)
            return `SELL setup is already sitting at or below support ${formatPrice(pairContext.support)}.`;
    ...
    if (distanceToResistance <= 0)
        return `BUY setup is already sitting at or above resistance ${formatPrice(pairContext.resistance)}.`;
...
function deriveExecutionConditions(symbol, pairContext, technicalContext) {
    ...
    const headlineRisk = pairContext.highImpactEvents.length > 0
        ? pairContext.highImpactEvents.some((event) => /in\s([0-5]?\d)m/i.test(event)) ? 'high' : 'medium'
        : pairContext.risks.some((risk) => /headline|geopolitic|war|conflict|breaking/i.test(risk)) ? 'medium' : 'low';
...
function buildObjectiveValidationFacts(args) {
    ...
    if (!orderTypeValid.status)
        hardRejectionReasons.push(orderTypeValid.reason);
    if (alreadyInvalidated.status)
        hardRejectionReasons.push(alreadyInvalidated.reason);
    if (parsed.stopLoss == null)
        hardRejectionReasons.push('Stop loss is missing, so invalidation and risk cannot be validated.');
    ...
    if (currentPrice == null)
        hardRejectionReasons.push('Current price is missing, so execution validity cannot be confirmed.');
    if (freshness.freshnessStatus === 'Expired')
        hardRejectionReasons.push(`Signal is ${freshness.freshnessStatus.toLowerCase()} based on age and/or price drift from entry.`);
    let rejectionCategory = 'NONE';
    if (alreadyInvalidated.status)
        rejectionCategory = 'ALREADY_INVALIDATED';
    else if (!orderTypeValid.status)
        rejectionCategory = 'INVALID_ORDER_TYPE';
    else if (parsed.stopLoss == null || ((parsed.orderType === 'LIMIT' || parsed.orderType === 'STOP') && parsed.entry == null) || currentPrice == null)
        rejectionCategory = 'INSUFFICIENT_DATA';
    else if (freshness.freshnessStatus === 'Stale' || freshness.freshnessStatus === 'Expired')
        rejectionCategory = 'STALE_SIGNAL';
...
function buildRiskRewardSummary(rr) {
    ...
    const tpAssessments = rr.targets.map((target) => {
        ...
        if (target.ratio >= 1.5)
            quality = 'good';
        else if (target.ratio < 0.5)
            quality = 'very_weak';
        else if (target.ratio < 1)
            quality = 'weak';
    ...
    if (best == null)
        overallQuality = 'unavailable';
    else if (tp1 != null && tp1 >= 1 && best >= 1.5)
        overallQuality = 'good';
    else if (best >= 1.5 || (tp1 != null && tp1 >= 0.5))
...
function deriveTechnicalAlignment(parsed, pairContext) {
    if (!pairContext || !parsed.direction)
        return 'unavailable';
    if (pairContext.directionBias === 'unknown' || pairContext.directionBias === 'neutral')
        return 'mixed';
    if (pairContext.directionBias === 'mixed')
        return 'mixed';
    if (parsed.direction === 'BUY')
        return pairContext.directionBias === 'bullish' ? 'aligned' : 'against';
    if (parsed.direction === 'SELL')
        return pairContext.directionBias === 'bearish' ? 'aligned' : 'against';
    return 'unavailable';
}
function deriveFundamentalAlignment(parsed, pairContext) {
    if (!pairContext || !parsed.direction)
        return 'unavailable';
    if (pairContext.macroBias === 'unknown' || pairContext.macroBias === 'neutral')
        return 'mixed';
    if (pairContext.macroBias === 'mixed')
        return 'mixed';
    if (parsed.direction === 'BUY')
        return pairContext.macroBias === 'bullish' ? 'aligned' : 'against';
    if (parsed.direction === 'SELL')
        return pairContext.macroBias === 'bearish' ? 'aligned' : 'against';
    return 'unavailable';
}
...
function buildLivePriceContext(pairContext) {
    if (!pairContext)
        return 'Live MT5-backed price unavailable.';
    return [
        `Current price: ${formatPrice(pairContext.price)}`,
...
function buildFallbackValidation(args) {
    ...
    const decisionLabel = hardReject ? 'REJECTED' : objectiveFacts.freshnessStatus === 'Delayed' ? 'NEEDS_CONFIRMATION' : objectiveFacts.freshnessStatus === 'Stale' ? 'WAIT' : 'WAIT';
...
function baseFundamentalAssessment(fundamentalsContext, alignment) {
    if (alignment === 'aligned')
        return 'AlphaMentals fundamentals support the signal direction.';
    if (alignment === 'against')
        return 'AlphaMentals fundamentals conflict with the signal direction.';
    if (alignment === 'mixed')
        return 'Fundamental intelligence is mixed.';
...
function applyHardRejectionOverride(candidate, objectiveFacts) {
    if (!objectiveFacts.hardRejectionRequired)
        return candidate;
    return {
        ...candidate,
        verdict: 'BAD',
...
async function validateTelegramTradeSignal(rawText, overrideParsed, meta) {
    const parsed = {
        ...(0, parseTelegramSignal_js_1.parseTelegramSignal)(rawText),
        ...overrideParsed,
    };
    if (!parsed.symbol)
        return { ok: false, error: 'Could not detect a trading symbol in this signal.' };
    if (!parsed.direction)
        return { ok: false, error: 'Could not detect trade direction (BUY/SELL) in this signal.' };
    ...
    try {
        pairContext = await (0, pairDecisionContext_service_js_1.getPairDecisionContext)(parsed.symbol);
...
If invalid order type, already invalidated, missing SL, missing entry, or expired signal is detected, verdict must be BAD and finalAction must be avoid.`;
    ...
    try {
        const response = await (0, gemini_js_1.chatCompleteJSON)([
